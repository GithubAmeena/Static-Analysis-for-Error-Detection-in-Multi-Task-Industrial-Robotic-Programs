...
//        FileReader fr = new FileReader("flex loader.txt");
        FileReader fr = new FileReader("C:\\Users\\Ameena\\Downloads\\task analyzer1\\Rapid-program-parser\\src\\Input-files\\rule4.txt"); // path to source rapid file
    ...
    private static boolean c3() throws IOException {
          ...
          FileReader fr1 = new FileReader("C:\\Users\\Ameena\\Downloads\\task analyzer1\\Rapid-program-parser\\src\\Input-files\\rule4.txt");
        ...
        for (int m = 0; m < parsedTasks.size() - 1; m++) {
            ...
            for (int n = m + 1; n < parsedTasks.size(); n++) {
                ...
                for (int i = 0; i < p1.syncInstructionsList.size(); i++) {
                    ArrayList<String> accessedVars1 = new ArrayList<>();
                    if (i > 0) {
                        int l1 = p1.syncInstructionsList.get(i - 1).getLineNumber();
                        int l2 = p1.syncInstructionsList.get(i).getLineNumber();
                        ...
                        int l1 = p1.syncInstructionsList.get(i).getLineNumber();
                        for (String s : sharedVariables.get(m)) {
                            ArrayList<Integer> readLocs = p1.varMap.get(s).getReadLocations();
                            if (readLocs != null && !readLocs.isEmpty()) {
                                int firstReadLoc = readLocs.get(0);
                 ...
                 //   System.out.println("Accessed Vars1 :" + accessedVars1);
                    ...
                    if (i < p2.syncInstructionsList.size() - 1) {
                        int l1 = p2.syncInstructionsList.get(i).getLineNumber();
                        int l2 = p2.syncInstructionsList.get(i + 1).getLineNumber();
